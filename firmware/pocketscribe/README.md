...
- **Capture:** physical button — short press = Quick Note, long press = Meeting. Records 16 kHz mono to microSD in chunks. Works offline.
- **Sync:** when Wi-Fi is up, upload unsynced chunks to the backend (`/ingest`), resumable + retryable; pull `/device/state` for the screen.
- **Display:** touch-navigated e-paper — Home/status, To-dos, Notes (title + category), Meetings. Partial refresh for timers/paging.
